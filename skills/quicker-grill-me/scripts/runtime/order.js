// Generated from dist/src/order.js by tools/sync-skill.mjs. Do not edit.
import { DECISION_STAGES } from "./types.js";
const stageRanks = new Map(DECISION_STAGES.map((stage, index) => [stage, index]));
function dependencyIds(question) {
...
export function orderQuestions(questions) {
    ...
    const compare = (left, right) => {
        const stageDifference = (stageRanks.get(left.stage) ?? 0) - (stageRanks.get(right.stage) ?? 0);
        if (stageDifference !== 0) {
            return stageDifference;
        }
        const topicDifference = (topicIndex.get(left.topic) ?? 0) - (topicIndex.get(right.topic) ?? 0);
