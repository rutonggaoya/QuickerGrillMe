// Generated from dist/src/review.js by tools/sync-skill.mjs. Do not edit.
...
function renderTable(header, rows) {
    ...
    const renderRow = (row, element) => {
        const normalized = Array.from({ length: width }, (_, index) => row[index] ?? "");
        return `<tr>${normalized
            .map((cell) => `<${element}>${renderInline(cell)}</${element}>`)
            .join("")}</tr>`;
    };
    return `<div class="table-wrap"><table><thead>${renderRow(header, "th")}</thead><tbody>${rows.map((row) => renderRow(row, "td")).join("")}</tbody></table></div>`;
}
function renderMarkdown(markdown) {
    ...
    const flushList = () => {
        if (listType !== undefined) {
            output.push(`<${listType}>${listItems
...
export function renderDesignReview(questionnaire, submission, designMarkdown, options = {}) {
    ...
    const decisions = orderQuestions(questionnaire.questions)
        ...
        .join("\n");
    const deferredCount = Object.values(submission.answers).filter((answer) => answer.status === "deferred").length;
    return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8">
  ...
  <body>
    <main>
      ...
      <section class="decisions">
        <h2>Questionnaire decision record</h2>
...
export async function persistDesignReview(outputPath, html) {
    ...
    try {
        ...
        try {
            ...
            throw new AggregateError([error, cleanupError], `Failed to persist design review and clean up ${temporaryPath}`);
        }
        throw error;
    }
}
