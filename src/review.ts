...
function optionLabels(question: Question, value: AnswerRecord["value"]): string {
...
function optionDescriptions(question: Question, value: AnswerRecord["value"]): string {
  const values = Array.isArray(value) ? value : [value];
...
function recommendedValue(question: Question): AnswerRecord["value"] {
...
function renderDecision(question: Question, answer: AnswerRecord): string {
  ...
  const delta = hasDelta
    ? `<div class="delta">
      <p><strong>Agent recommendation:</strong> ${escapeHtml(
        optionLabels(question, recommendation)
      )} <span>(${escapeHtml(question.recommendationConfidence)} confidence)</span></p>
