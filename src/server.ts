...
export interface QuestionnaireServerOptions {
  ...
  exitOnPageClose?: boolean;
  pageCloseGracePeriodMs?: number;
  onSubmitted?: (outputPath: string) => void;
  onPageClosed?: () => void;
}

export interface RunningQuestionnaireServer {
  url: string;
  closed: Promise<void>;
  close: () => Promise<void>;
}
...
async function sendStatic(response: ServerResponse, fileName: string): Promise<void> {
...
function parsePreviewRequest(value: unknown): {
  level: DepthLevel;
  ...
  const answers: Record<string, AnswerValue> = Object.create(null);
...
export function createQuestionnaireServer(options: QuestionnaireServerOptions): Server {
  const requestToken = randomBytes(32).toString("hex");
  let pageCloseTimer: NodeJS.Timeout | undefined;
  let server: Server;
  ...
  function cancelPendingPageClose(): void {
  ...
  function schedulePageClose(): void {
...
export async function startQuestionnaireServer(
  options: QuestionnaireServerOptions
): Promise<RunningQuestionnaireServer> {
  const server = createQuestionnaireServer(options);
  const port = options.port ?? 0;
  await new Promise<void>((resolveStarted, rejectStarted) => {
    const onError = (error: Error) => rejectStarted(error);
